...
class WeChatOrder(models.Model):
    ...
    def _create_jsapi_order(self, lines, create_vals):
        ...
        """
        debug = (
            self.env["ir.config_parameter"].sudo().get_param("wechat.local_sandbox")
            == "1"
        )
        ...
        result_json = mpay.jsapi.get_jsapi_params(
            prepay_id=result_raw.get("prepay_id"), nonce_str=result_raw.get("nonce_str")
        )
        ...
        return {"order_id": order.id, "data": result_json}
