...
class WechatMiniProgramController(http.Controller):
    @http.route(
        "/wechat/miniprogram/authenticate", type="json", auth="public", csrf=False
    )
    def authenticate(self, code, user_info, test_cr=False):
        ...
        """
        _logger.debug(
            "/wechat/miniprogram/authenticate request: code - %s, user_info - %s",
            code,
            user_info,
        )
        openid, session_key = self.get_openid(code)
        _logger.debug(
            "Authenticate on WeChat server: openid - %s, session_key - %s",
            openid,
            session_key,
        )
        ...
        if not openid or not session_key:
            raise UserError(
                _(
                    "Unable to get data from WeChat server : openid - %s, session_key - %s"
                )
                % (openid, session_key)
            )
        ...
        if user:
            user.write({"wechat_session_key": session_key})
        else:
            country = request.env["res.country"].search(
                [("name", "like", "%" + user_info.get("country") + "%")], limit=1
            )
            ...
            user = User.create(
                {
                    ...
                    "groups_id": [
                        (
                            4,
                            request.env.ref(
                                "wechat_miniprogram.group_miniprogram_user"
                            ).id,
                        )
                    ],
                }
