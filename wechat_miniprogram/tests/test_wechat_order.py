...
class TestWeChatOrder(HttpCase):
    ...
    def url_open_json(self, url, code, user_info, timeout=10):
        ...
        params = {"context": {}, "code": code, "user_info": user_info, "test_cr": True}
        data = {"jsonrpc": "2.0", "method": "call", "params": params, "id": None}
    ...
    def test_authenticate_miniprogram_user(self):
        ...
        session_info = self._authenticate_miniprogram_user(code, user_info).get(
            "result"
        )
        self.assertIn(
            "session_id",
            session_info,
            'Authenticate user: "session_id" not found in data',
        )
