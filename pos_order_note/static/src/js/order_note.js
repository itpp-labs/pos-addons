...
odoo.define("pos_order_note", function(require) {
    ...
    models.Order = models.Order.extend({
        ...
        computeChanges: function(categories, config) {
            ...
            for (line_hash in current_res) {
                if (Object.prototype.hasOwnProperty.call(current_res, line_hash)) {
                    ...
                    if (
                        cancelled_exist_change &&
                        curr.old_custom_notes &&
                        curr.old_custom_notes !== curr.custom_notes
                    ) {
        ...
        change_custom_notes: function() {
            ...
            if (
                current_order_custom_notes_ids.length ===
                old_order_custom_notes_ids.length
            ) {
                var difference = this.DiffArrays(
                    current_order_custom_notes_ids,
                    old_order_custom_notes_ids
                );
    ...
    var ProductNotesPopupWidget = PopupWidget.extend({
        ...
        set_available_notes: function() {
            ...
            if (type === "Product") {
                ...
                var category_ids = [];
                if (
                    orderline.product.pos_categ_id &&
                    orderline.product.pos_categ_id.length
                ) {
        ...
        get_notes_by_category_ids: function(category_ids) {
            ...
            function get_parent_category_ids(child_category_id) {
                ...
                var parent_category_id = self.pos.db.get_category_parent_id(
                    child_category_id
                );
            ...
            category_ids.forEach(function(id) {
                all_categories_ids = all_categories_ids.concat(
                    get_parent_category_ids(id)
                );
            });
