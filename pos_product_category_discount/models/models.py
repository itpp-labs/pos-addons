...
class PosCategoryDiscount(models.Model):
    ...
    category_discount_pc = fields.Float(
        string="Discount Percentage", default=10, help="The default discount percentage"
    )
    ...
    discount_program_id = fields.Many2one(
        "pos.discount_program", string="Discount Program"
    )
...
class PosDiscountProgram(models.Model):
    ...
    discount_program_active = fields.Boolean(
        string="Active",
        default=True,
        help="Activate or deactivate the discount program in POS",
    )
    discount_category_ids = fields.One2many(
        "pos.category_discount", "discount_program_id", string="Discount Category"
    )
...
class ProductTemplate(models.Model):
    ...
    discount_allowed = fields.Boolean(
        ...
        help="Check if you want this product discount allowed in the Point of Sale",
        default=True,
    )
...
class ResPartner(models.Model):
    ...
    discount_program_id = fields.Many2one(
        "pos.discount_program",
        string="Discount Program",
        help="That programm will be applied when customer is selected",
    )
