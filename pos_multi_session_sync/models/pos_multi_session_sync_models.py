...
class PosConfigSync(models.Model):
    ...
    multi_session_ID = fields.Integer(
        "Multi-session",
        index=True,
    ...
    multi_session_message_ID = fields.Integer(
        index=True, default=1, string="Last sent Multi-session message number"
    )
...
class PosMultiSessionSync(models.Model):
    ...
    run_ID = fields.Integer(
        index=True, string="Session", help="Actual run_id of the multi_session"
    )
    ...
    def check_order_revision(self, message, order):
        ...
        _logger.debug(
            "Client revision ID %s: Server revision ID %s",
            client_revision_ID,
            server_revision_ID,
        )
        _logger.debug(
            "Client nonce %s: Server nonce %s", message["data"]["nonce"], order.nonce
        )
    ...
    def set_changes(self, message, order):
        ...
        lines_list = {line[2]["uid"]: line[2] for line in lines}
    ...
    def set_and_broadcast_order(self, message):
        ...
        return {
            ...
            "uid": order.order_uid,
            "order_ID": updated_message["data"]["sequence_number"],
            "run_ID": order.run_ID,
        }
    ...
    def set_order(self, message):
        ...
        order = self.env["pos_multi_session_sync.order"].search(
            [("order_uid", "=", order_uid)]
        )
    ...
    def get_sync_all(self, message):
        ...
        pos.write({"multi_session_message_ID": message_ID})
        if uid:
            ...
            for order in self.env["pos_multi_session_sync.order"].search(
                [
                    ("multi_session_ID", "=", self.multi_session_ID),
                    ("state", "=", "draft"),
                    ("run_ID", "=", run_ID),
    ...
    def send_sync_message(self, message):
        ...
        for pos in self.env["pos_multi_session_sync.pos"].search(
            [
                ("user_ID", "=", self.env.context.get("user_ID")),
                ("multi_session_ID", "=", self.multi_session_ID),
            ]
            ...
            self.env["pos.config"]._send_to_channel_by_id(
                self.dbname, pos.pos_ID, channel_name, message
            )

        if self.env.context.get("phantomtest") == "slowConnection":
            _logger.info(
                "Delayed notifications from %s: %s", self.env.user.id, notifications
            )
