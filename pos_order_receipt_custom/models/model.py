...
class RestaurantPrinter(models.Model):
    ...
    custom_order_receipt_id = fields.Many2one(
        "pos.custom_receipt",
...
class PosConfig(models.Model):
    ...
    print_transfer_info_in_kitchen = fields.Boolean(
        string="Print Order Transfer Ticket",
        help="Print the kitchen ticket once the order is transfered to another table",
        default=True,
    )
    custom_kitchen_receipt = fields.Boolean(
        compute="_compute_custom_kitchen_receipt",
        readonly=True,
        string="Using of Custom Kitchen Receipt",
    )
