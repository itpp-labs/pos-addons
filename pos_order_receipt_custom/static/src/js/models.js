...
odoo.define("pos_order_receipt_custom.models", function(require) {
    ...
    models.PosModel = models.PosModel.extend({
        initialize: function() {
            ...
            receipt_model.condition = function(self) {
                return (
                    self.config.custom_ticket ||
                    self.config.custom_xml_receipt ||
                    self.config.custom_kitchen_receipt
                );
            };
        ...
        set_table: function(table) {
            ...
            if (
                table &&
                order &&
                !order.first_order_printing &&
                this.config.print_transfer_info_in_kitchen &&
                !order.printed_transfer
            ) {
                this.print_transfer_order(
        ...
        print_transfer_order: function(order, old_table, new_table) {
            ...
            if (old_table.id !== new_table.id) {
                ...
                this.printers.forEach(function(printer) {
                    var products = self.get_order_product_list_of_printer(
                        printer,
                        order
                    );
        ...
        get_order_product_list_of_printer: function(printer, order) {
            ...
            var lines = orderlines.filter(function(line) {
                ...
                if (
                    ...
                    line.product.pos_category_ids.length &&
                    line.mp_dirty === false
                ) {
                    var res = printer.config.product_categories_ids.filter(function(
                        cat
                    ) {
                ...
                if (
                    self.is_product_in_product_list &&
                    self.is_product_in_product_list(
                        line.product.id,
    ...
    models.Order = models.Order.extend({
        ...
        get_custom_order_receipt: function(printer, changes) {
            ...
            var receipt_template = this.get_receipt_template_by_id(
                printer.config.custom_order_receipt_id[0],
                "order_receipt"
            );
            var parser = new DOMParser();
            var xmlDoc = parser.parseFromString(
                receipt_template.qweb_template,
                "text/xml"
            );

            var receipt = this.custom_qweb_render(xmlDoc.documentElement, {
                changes: changes,
                widget: this,
            });
            return receipt;
