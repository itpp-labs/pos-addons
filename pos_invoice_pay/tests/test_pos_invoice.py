...
class TestModel(odoo.tests.SingleTransactionCase):
    def test_pay_more_than_enough(self):
        class MockedAccountPayment:
            ...
            # in process_invoice_payment method
            # following methods are executed on "account.payment.register" model
            sudo = with_context = _create_payments = write = _return_self

        class MockedAccountInvoice:
            def __init__(self, residual, currency):
            ...
            def browse(self, arg=None, prefetch=None):
                ...
                mock.currency_id = self.currency
                return mock
        ...
        mocked_env = MockedEnv(
            self.env,
            {
                "account.payment.register": mocked_account_payment,
        ...
        sample.env["account.move"] = MockedAccountInvoice(due, self.env.ref("base.USD"))
