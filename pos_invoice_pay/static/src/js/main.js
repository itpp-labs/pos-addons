...
odoo.define("pos_invoices", function(require) {
    ...
    var SaleOrdersWidget = InvoicesAndOrdersBaseWidget.extend({
        ...
        create_invoice: function(sale_order) {
            var self = this;
            new Model("pos.order")
                .call("process_invoices_creation", [sale_order.id])
                .then(
                    function(created_invoice_id) {
                        self.pos
                            .update_or_fetch_invoice(created_invoice_id)
                            .then(function(new_invoice_id) {
                                self.render_data(self.pos.db.sale_orders);
                                self.pos
                                    .validate_invoice(new_invoice_id)
                                    .then(function(validated_invoice_id) {
                                        self.pos
                                            .update_or_fetch_invoice(
                                                validated_invoice_id
                                            )
                                            .then(function(res) {
