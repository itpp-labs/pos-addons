...
odoo.define('pos_invoices', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        initialize: function (session, attributes) {
            ...
            this.add_channel("pos_sale_orders", this.on_invoice_pay_notification, this);
            this.add_channel("pos_invoices", this.on_invoice_pay_notification, this);
        },
        ...
        get_sale_order_lines: function (ids) {
            var self = this,
                def = $.Deferred();
            this.get_lines(ids, 'sale.order', 'get_order_lines_for_pos').then(function (lines) {
        ...
        get_invoice_lines: function (data) {
            var self = this;
            var def = $.Deferred();
            this.get_lines(data, 'account.invoice', 'get_invoice_lines_for_pos').then(function (lines) {
                var inv = {},
                    inv_id = 0,
                    max = lines.length,
                    i = 0;
        ...
        on_invoice_pay_notification: function(notification) {
        ...
        get_res: function (model_name, id) {
            ...
            return new Model(model_name).query(fields).filter([['id', '=', id]]).all();
        },
        ...
        validate_invoice: function (id) {
            var result = $.Deferred();
            new Model('account.invoice').exec_workflow(id[0], 'invoice_open').then(function (res) {
                if (res) {
                    result.resolve(id);
                } else {
                    result.reject();
        ...
        create_invoice: function (sale_order) {
            ...
            new Model('pos.order').call('process_invoices_creation', [sale_order.id]).then(function (created_invoice_id) {
                self.pos.update_or_fetch_invoice(created_invoice_id).then(function (new_invoice_id) {
                    ...
                    self.pos.update_or_fetch_invoice(new_invoice_id).then(function(res) {
        ...
        click_next: function () {
            ...
            if (this.selected_invoice) {
                ...
                switch (this.selected_invoice.state) {
                    case "Draft":
                        this.pos.validate_invoice(this.selected_invoice.id).then(function (id) {
        ...
        finalize_validation: function () {
            ...
            if (order.is_to_invoice()) {
                this.pos.push_order(order).then(function () {
                    ...
                    new Model('account.invoice').call('invoice_print', [order.invoice_to_pay.id]).then(function (action) {
