odoo.define("pos_mobile.screens", function(require) {
    ...
    screens.ProductCategoriesWidget.include({
        init: function(parent, options) {
            ...
            this.switch_category_handler = function(event) {
                self.set_category(
                    self.pos.db.get_category_by_id(Number(this.dataset.categoryId))
                );
                self.renderElement();
        ...
        renderElement: function() {
            ...
            this.el
                .querySelector(".slide-categories-button")
                .addEventListener("click", this.click_categories_slide);
            this.el
                .querySelector(".slide-numpad-button")
                .addEventListener("click", this.click_numpad_slide);
            ...
            if (
                this.category &&
                this.category.child_id &&
                !this.category.child_id.length
            ) {
                if (active_category_id === this.category.id) {
                    if (this.category.parent_id) {
                        this.set_category(
                            this.pos.db.get_category_by_id(this.category.parent_id[0])
                        );
                        this.renderElement();
                    } else {
                        this.reset_category();
                    ...
                    $(
                        ".categories span[data-category-id=" + this.category.id + "]"
                    ).addClass("active");
                }
            } else {
        ...
        get_image_url: function(category) {
            return (
                ...
                "/web/image?model=pos.category&field=image&id=" +
                category.id
            );
        },
    });
    ...
    screens.ProductScreenWidget.include({
        click_product: function(product) {
            this._super.apply(this, arguments);
            var $qty = $(
                'article[data-product-id="' + product.id + '"] .current-order-qty'
            );
    ...
    screens.OrderWidget.include({
        ...
        change_product_qty: function(product_id) {
            ...
            if (order) {
                ...
                var products = this.pos.gui.screen_instances.products
                    .product_list_widget.product_list;
