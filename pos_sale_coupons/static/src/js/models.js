...
odoo.define("pos_sale_coupons.models", function(require) {
        ...
        on_coupons_notification: function(message) {
            ...
            if (coupon_programs) {
                _.each(coupon_programs, function(coupon_program) {
                    var exist_sale_coupon_program = self.db.get_sale_coupon_program_by_id(
                        coupon_program.id
                    );
    ...
    models.Order = models.Order.extend({
        add_product: function(product, options) {
            ...
            var coupon_orderline = orderlines.find(function(line) {
                return (
                    line.coupon &&
                    product.coupon &&
                    line.coupon.id === product.coupon.id
                );
            });
            if (coupon_orderline) {
                return false;
        ...
        apply_discount_program: function(coupon, program) {
            var discount_product = this.pos.db.product_by_id[
            ...
            if (program.discount_type === "fixed_amount") {
                ...
                discount_price = this.get_amount_discount_price(
                    program.discount_percentage
                );
                if (program.discount_max_amount) {
                    discount_price = Math.min(
                        discount_price,
                        program.discount_max_amount
                    );
                }
                ...
                discount_price =
                    cheapest_product_orderline.get_display_price() *
                ...
                var specific_product = this.pos.db.product_by_id[
                    program.discount_specific_product_id[0]
                ];
                if (specific_product) {
                    // Calc price of the coupon by specific product price
                    var specific_product_orderline = this.get_orderline_by_product(
                        specific_product
                    );
                    if (specific_product_orderline) {
                        discount_price =
                            specific_product_orderline.get_display_price() *
                            (program.discount_percentage / 100);
                        if (program.discount_max_amount) {
                            discount_price = Math.min(
                                discount_price,
                    ...
                    return this.pos.gui.show_popup("error", {
                        ...
                        body: _t(
                            "The coupon product is used which is not available in the POS."
                        ),
                    });
                }
            }
        ...
        apply_product_reward_program: function(coupon, program) {
            var reward_product = this.pos.db.product_by_id[
                program.reward_product_id[0]
            ];
            if (reward_product) {
                ...
                // Add free product
                this.add_product(reward_product, {
                    quantity: program.reward_product_quantity,
                    price: 0,
                });
                this.pos.db.update_old_coupon_ids(coupon.id);
            } else {
        ...
        get_cheapest_product_orderline: function() {
            ...
            var sorted_orderlines = orderlines.sort(function(x, y) {
                return (
                    x.product.get_price(self.pricelist, x.get_quantity()) -
    ...
    models.Orderline = models.Orderline.extend({
        initialize: function(attr, options) {
            ...
            if (
                options.product &&
                options.product.coupon &&
                options.product.coupon.id
            ) {
                this.coupon = {
        ...
        can_be_merged_with: function(orderline) {
            if (
                this.coupon &&
                orderline.coupon &&
                this.coupon.id !== orderline.coupon.id
            ) {
                return false;
            }
        ...
        set_unit_price: function(price) {
            if (this.coupon) {
                var coupon_price =
                    this.coupon.coupon_value && parseInt(this.coupon.coupon_value);
