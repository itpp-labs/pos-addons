...
class SaleCouponProgram(models.Model):
    ...
    force_sale_before_consumption = fields.Boolean(
        string="Force sale the coupon",
        default=True,
        help="Force sale the coupons before consumption" "(for POS only)",
    )
    ...
    def _compute_order_count(self):
        for program in self:
            orders = program.coupon_ids.filtered(
                lambda c: c.pos_order_id is not False
            ).mapped("pos_order_id")
    ...
    def action_view_pos_orders(self):
        self.ensure_one()
        orders = self.coupon_ids.filtered(lambda c: c.pos_order_id is not False).mapped(
            "pos_order_id"
        )
        return {
            "name": _("POS Orders"),
...
class SaleCoupon(models.Model):
    ...
    pos_discount_line_product_id = fields.Many2one(
        "product.product",
        related="program_id.pos_product_id",
        readonly=False,
        help="Product used in the sales order to apply the discount.",
    )
    pos_order_id = fields.Many2one(
        "pos.order",
        "Applied on POS order",
        readonly=True,
        help="The POS order on which the coupon is applied",
    )
    sold_via_order_id = fields.Many2one(
        "pos.order",
        string="Sold via",
        readonly=True,
    ...
    def _check_coupon_code(self, order):
        ...
        if self.expiration_date and self.expiration_date < order.date_order.date():
            message = {
                "error": _("This coupon %s has been used or is expired.") % (self.code)
            }
        # Minimum requirement should not be checked if the coupon got generated by a promotion program (the requirement should have only be checked to generate the coupon)
            ...
            self.program_id.program_type == "coupon_program"
            and not self.program_id._filter_on_mimimum_amount(order)
        ):
            message = {
                "error": _("A minimum of %s %s should be purchased to get the reward")
                % (
            ...
            self.program_id._is_global_discount_program()
            and order._is_global_discount_already_applied()
        ):
            message = {"error": _("Global discounts are not cumulable.")}
        elif (
            self.program_id.reward_type == "product"
            and not order._is_reward_in_order_lines(self.program_id)
        ):
            message = {
                "error": _(
        ...
        # Product requirement should not be checked if the coupon got generated by a promotion program (the requirement should have only be checked to generate the coupon)
            ...
            self.program_id.program_type == "coupon_program"
            and not self.program_id._filter_programs_on_products(order)
        ):
            message = {
                "error": _(
            ...
            if (
                self.program_id not in applicable_programs
                and self.program_id.promo_applicability == "on_current_order"
            ):
                message = {
                    "error": _(
