        ...
        if vals.get("coupon_id"):
            ...
            if vals.get("coupon_state") == "sold":
                ...
                if vals.get("coupon_value"):
                    updated_coupon_vals["program_id"] = self.process_new_program(
                        coupon, vals.get("coupon_value")
                    ).id
                updated_coupon_vals.update(
                    {
                        "state": "reserved",
                        "partner_id": res.order_id.partner_id.id
                        if res.order_id.partner_id
                        else False,
                ...
                # coupon consumed
                coupon.write({"state": "used", "pos_order_id": res.order_id.id})
        return res

    @api.model
    def process_new_program(self, coupon, value):
        base_prog = coupon.program_id
        current_prog = base_prog.search(
            [
