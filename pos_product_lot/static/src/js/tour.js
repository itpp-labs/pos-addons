odoo.define('pos_product_lot.tour', function(require) {
"use strict";
    ...
    function change_qty_with_numpad(qty) {
        return generate_keypad_steps(qty, '.pads');
    }
    ...
    steps = steps.concat(add_product_to_order('Boni Oranges'));

    steps = steps.concat(change_qty_with_numpad("3"));

    steps = steps.concat({
        content: "Unpack corresponding lot",
        trigger: '.unpack-lot',
        position: "bottom"
    });
    ...
    steps = steps.concat(generate_payment_screen_keypad_steps("5.94"));

    steps = steps.concat(finish_order());

    steps = steps.concat({
        trigger: '.product .not-available:contains("-1")',
        run: function () {
            // it's a check
        },
    });
    ...
    tour.register('pos_product_lot_tour', {test: true, url: '/web' }, steps);
});
